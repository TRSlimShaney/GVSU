...
public class Encryption {
     ...
     *******************************************************************/
    public List getClipBoard( int n )
    {
     ...
     *    decryption.
     *******************************************************************/
    public void add(String str, int index) throws Exception
    {
     ...
     *******************************************************************/
    public void add( List clipBD, int index ) throws Exception {
     ...
     *******************************************************************/
    public void cut(int start, int stop) throws Exception {
     ...
     *******************************************************************/
    public void copy(int start, int stop) throws Exception
    {
 // 3A) fix the condition to include all out of range exceptions
 
        if (start < 0 || stop < 0 || start > stop || stop >= message.size())
        {
...
// 3B) one line of code 
    ...
    public void delete(String ch)
    {
        ...
        while ((index = message.indexOf(ch)) != -1)
        {
            ...
            undoGroup.push(new Command("a " + ch + " " + (index)));
            // 4) one line of code to create the undo command
     ...
     * ************************************************************************************/
    public void obfuscate( int times ) throws Exception
    {
    	Stack undoGroup = new Stack("new undo commands ");
        String ch;
        int index;
     ...
     *******************************************************************/
    public void paste(int clipNum, int index) throws Exception
    {
 // 6A) fix the condition to include all out of range exceptions

        if (index < 0 || index > message.size() - 1 || clipBoards.get(clipNum) == null || clipBoards.size() <= clipNum || clipNum < 0)
        {
            throw new Exception("Paste: invalid clipboard number");
        }
        
        add((List) clipBoards.get(clipNum), index);
        //message.addAll(index, (List) clipBoards.get(clipNum));
        //message.add(index, clipBoards.get(clipNum).toString());
     ...
     *     decryption.
     *******************************************************************/
    public void remove(int start, int stop) throws Exception
    {
        Stack undoGroup = new Stack("new undo commands ");
        String str;
 ...
 // 7A) fix the condition to include all out of range exceptions
        ...
        int length5 = stop - start;
        for (int n = 0; n <= length5; n++)
        {
        	String tempString = message.get(start).toString();
            message.remove(start);
            // 1) fix with one line of code
            undoGroup.push(new Command("a " + tempString + " " + (start)));
        }
     ...
     *     decryption.
     * *****************************************************************/
    public void swap(int j, int k) throws Exception
    {
        Stack undoGroup = new Stack("new undo commands ");

 // 9A) fix the condition to include all out of range exceptions
        if (j < 0 || k < 0 || k >= message.size() || j >= message.size())
        {
     ...
     * command group
     * *****************************************************************/
    public void undoPreviousEncryption()
    {
        Stack previousCommandGroup;
     ...
     * *****************************************************************/
    public void undoEncryptionHistory()
    {
